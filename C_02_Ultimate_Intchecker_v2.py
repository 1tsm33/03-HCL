...
while guess != secret and guesses_used < guesses_allowed:
    ...
    if guess < secret and guesses_used < guesses_allowed:
        feedback = (" Too low, please try a higher number ‼️."
                    f" You've used {guesses_used} / {guesses_allowed} guesses")
    elif guess > secret and guesses_used < guesses_allowed:
        feedback = (" Too high, please try a lower number 🫤."
                    f" You've used {guesses_used} / {guesses_allowed} guesses")

    # when the secret number is guessed, we have tree different feedback
    # options (lucky / that was close / well done)
    elif guess == secret:
        ...
        if guesses_used == 1:
            ...
            feedback = (f" Well done! you guessed he number in {guesses_used} guesses.")

    # if there are no guesses left!
    else:
        feedback = "Sorry you got no more guesses. You lost 🎀womp womp🎀."

        # print feedback to user
        print(feedback)

        # additional feedback (warn user that they are running outn of guesses)
